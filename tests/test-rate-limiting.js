...
async function testScopeCoverage() {
  ...
  for (const endpoint of inScopeEndpoints) {
    ...
    console.log(`Endpoint ${endpoint}: HTTP ${result.status}, headers ${result.hasLimitHeaders ? '✅' : '❌'}`);
...
async function testHealthExcluded() {
  ...
  console.log(`/health: HTTP ${result.status}, limiter headers ${hasLimitHeaders ? 'present ❌' : 'absent ✅'}`);
...
async function runTests() {
  ...
  console.log(`\n${results.scopeCoverage ? '✅' : '❌'} Scope Coverage (/api, /chat, /manual)`);
